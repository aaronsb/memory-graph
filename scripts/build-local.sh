...
# Skip linting as it's not defined in package.json
...
if [ "$VERBOSE" = true ]; then
    if docker build -t memory-graph:local .; then
    ...
    if docker build -t memory-graph:local . > "$DOCKER_LOG" 2>&1; then
...
echo -e "\n${GREEN}Build complete!${NC} Image tagged as memory-graph:local"
